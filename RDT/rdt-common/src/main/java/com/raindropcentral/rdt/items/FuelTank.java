...
/**
 * Bound Security fuel tank item and placed chest payload helper.
 *
 * @author ItsRainingHP
 * @since 1.0.0
 * @version 1.0.0
...
public final class FuelTank {
    ...
    private static final String ITEM_KIND = "fuel_tank";
    ...
    /**
     * Creates a bound Security fuel tank item.
     *
     ...
     * @return bound fuel tank chest item
     */
    public static @NotNull ItemStack getFuelTankItem(
        ...
        final ItemStack fuelTank = new ItemStack(Material.CHEST);
        final ItemMeta meta = Objects.requireNonNull(fuelTank.getItemMeta(), "fuel tank meta");
        meta.displayName(new I18n.Builder("fuel_tank.name", player)
            ...
            .build()
            .component());
        meta.lore(new I18n.Builder("fuel_tank.lore", player)
            .withPlaceholders(Map.of(
                "world", worldName,
                "chunk_x", chunkX,
                "chunk_z", chunkZ
        ...
        writeBinding(plugin, data, townUuid, worldName, chunkX, chunkZ);
        fuelTank.setItemMeta(meta);
        return fuelTank;
    }

    /**
     * Returns whether an item is a bound fuel tank payload.
     *
     * @param plugin active plugin runtime
     * @param item item to inspect
     * @return {@code true} when the item is a bound fuel tank
     */
    ...
    /**
     * Returns whether a placed chest state is a bound fuel tank.
     *
     * @param plugin active plugin runtime
     * @param chest placed chest to inspect
     * @return {@code true} when the chest is a bound fuel tank
     */
    public static boolean isPlacedFuelTank(final @NotNull RDT plugin, final @Nullable Chest chest) {
        return chest != null && isPlacedFuelTank(plugin, chest.getPersistentDataContainer());
    }

    /**
     * Binds a placed chest to one Security chunk using the chest state PDC.
     *
     * @param plugin active plugin runtime
     ...
     * @param chunkZ owning chunk Z coordinate
     */
    public static void bindPlacedFuelTank(
        final @NotNull RDT plugin,
    ...
    private static boolean isPlacedFuelTank(
        final @NotNull RDT plugin,
