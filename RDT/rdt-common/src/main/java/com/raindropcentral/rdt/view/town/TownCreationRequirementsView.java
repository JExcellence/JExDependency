...
/**
 * Requirement view for the dedicated town-creation flow.
 *
 * @author ItsRainingHP
 * @since 1.0.0
 * @version 1.0.0
 */
public final class TownCreationRequirementsView extends BaseView {
    ...
    /**
     * Creates the creation requirement view.
     */
    public TownCreationRequirementsView() {
        super();
    }

    @Override
    protected @NotNull String getKey() {
        return "town_creation_requirements_ui";
    }
    ...
    public void onResume(final @NotNull Context origin, final @NotNull Context target) {
        final Map<String, Object> targetData = TownCreationViewSupport.copyInitialData(target);
        final Map<String, Object> data = targetData != null
            ? targetData
            : TownCreationViewSupport.copyInitialData(origin);
        if (data != null && data.get(TownCreationViewSupport.CONTRIBUTION_STATUS_KEY) instanceof String statusKey) {
    ...
    private void handleRequirementClick(
        ...
        if (requirement.kind() == RequirementKind.CURRENCY) {
            clickContext.openForPlayer(
                TownCreationCurrencyContributionAnvilView.class,
                TownCreationViewSupport.mergeInitialData(
                    clickContext,
                    Map.of(TownCreationViewSupport.ENTRY_KEY, requirement.entryKey())
                )
            );
            return;
        }
        ...
        final RDT plugin = TownCreationViewSupport.plugin(clickContext);
        ...
        final ContributionResult result = plugin.getTownRuntimeService().contributeTownCreationItem(
    ...
    private void sendContributionMessage(
        ...
        placeholders.put("contributed_amount", data.getOrDefault(TownCreationViewSupport.CONTRIBUTION_AMOUNT_KEY, 0));
        this.sendSharedMessage(player, statusKey, placeholders);
        if (Boolean.TRUE.equals(data.get(TownCreationViewSupport.CONTRIBUTION_COMPLETED_KEY))) {
            this.sendSharedMessage(player, "requirement_complete", placeholders);
        }
        if (Boolean.TRUE.equals(data.get(TownCreationViewSupport.READY_TO_CREATE_KEY))) {
    ...
    private @NotNull Map<String, Object> createNavigationData(final @NotNull Context context) {
        ...
        if (copiedData != null) {
            data.putAll(TownCreationViewSupport.stripTransientData(copiedData));
        }
        return data;
    }
    ...
    private void sendSharedMessage(
        ...
        new I18n.Builder("town_creation_shared.messages." + key, player)
            .includePrefix()
