...
/**
 * Anvil input view for partial currency turn-ins during town creation.
 *
 * @author ItsRainingHP
 * @since 1.0.0
 * @version 1.0.0
 */
public final class TownCreationCurrencyContributionAnvilView extends AbstractAnvilView {

    /**
     * Creates the town-creation currency contribution input view.
     */
    public TownCreationCurrencyContributionAnvilView() {
        super(TownCreationRequirementsView.class);
    }
    ...
    protected @Nullable Object processInput(final @NotNull String input, final @NotNull Context context) {
        ...
        final RDT plugin = TownCreationViewSupport.plugin(context);
        ...
        return plugin.getTownRuntimeService().contributeTownCreationCurrency(
            context.getPlayer(),
            entryKey,
            requestedAmount
        );
    ...
    protected @NotNull Map<String, Object> prepareResultData(
        ...
        if (copiedData != null) {
            resultData.putAll(TownCreationViewSupport.stripTransientData(copiedData));
        }
        resultData.put(TownCreationViewSupport.CONTRIBUTION_STATUS_KEY, this.resolveStatusKey(contributionResult.status()));
        resultData.put(TownCreationViewSupport.CONTRIBUTION_AMOUNT_KEY, contributionResult.contributedAmount());
        resultData.put(TownCreationViewSupport.CONTRIBUTION_COMPLETED_KEY, contributionResult.requirementCompleted());
        resultData.put(TownCreationViewSupport.READY_TO_CREATE_KEY, contributionResult.levelReady());
        return resultData;
    }

    private @Nullable String resolveEntryKey(final @NotNull Context context) {
        final Map<String, Object> data = TownCreationViewSupport.copyInitialData(context);
        return data != null && data.get(TownCreationViewSupport.ENTRY_KEY) instanceof String entryKey
            ? entryKey
            : null;
    }
