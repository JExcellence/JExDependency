...
public class FreeOneblockService implements IOneblockService {

    private static final Logger LOGGER = Logger.getLogger("JExOneblock");
    private static final String WORLD_NAME = "oneblock_world";
    ...
    public @NotNull CompletableFuture<Void> createIsland(@NotNull Player player) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                var existingPlayer = oneblockPlayerRepository.findByUuid(player.getUniqueId());
                if (existingPlayer != null && existingPlayer.getOwnedIsland() != null) {
                    throw new RuntimeException("Player already has an island");
                }

                World world = getOrCreateOneblockWorld();
                if (world == null) {
                    throw new RuntimeException("Failed to create oneblock world");
                }
                ...
                CompletableFuture<IslandRegion> regionFuture = regionManager.createIslandRegion(
                    player.getUniqueId(), 
                    world, 
                    100 // Free islands get standard radius
                );
                ...
                String islandIdentifier = "island_" + nextIslandId;

                var island = OneblockIsland.builder()
                        .owner(playerRecord)
                        .identifier(islandIdentifier)
                        .islandName(player.getName() + "'s Island")
                        .description("Welcome to " + player.getName() + "'s island!")
                        .centerLocation(islandLocation.clone())
                        .currentSize(100) // Match region radius
                        .maximumSize(200)
                        .level(1)
                        .experience(0.0)
                        .privacy(false)
                ...
                OneblockIsland savedIsland = island;
                oneblockPlayerRepository.findByUuidAsync(player.getUniqueId())
                        .thenCompose(optPlayer -> {
                            if (optPlayer.isPresent()) {
                                optPlayer.get().setOwnedIsland(savedIsland);
                                return oneblockPlayerRepository.updateAsync(optPlayer.get());
                            }
                            return CompletableFuture.completedFuture(null);
                        }).join();

                generateInitialOneblock(world, islandLocation);

                playerIslandCache.put(player.getUniqueId(), islandIdentifier);
                nextIslandId++;

                LOGGER.info("Created island " + island.getIdentifier() + " for player " + player.getName());
                return null;
            } catch (Exception e) {
                LOGGER.severe("Failed to create island for player " + player.getName() + ": " + e.getMessage());
    ...
    public @NotNull CompletableFuture<Boolean> deleteIsland(@NotNull UUID playerId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                LOGGER.info("Deleted island " + island.getIdentifier() + " for player " + playerId);
                return true;
            } catch (Exception e) {
                LOGGER.severe("Failed to delete island for player " + playerId + ": " + e.getMessage());
                return false;
            }
        });
    }
    ...
    public @NotNull CompletableFuture<Boolean> teleportToIsland(@NotNull Player player) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                LOGGER.severe("Failed to teleport player " + player.getName() + " to island: " + e.getMessage());
                return false;
            }
        });
    }
    ...
    public @NotNull CompletableFuture<Integer> getBlockLevel(@NotNull UUID playerId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                LOGGER.severe("Failed to get block level for player " + playerId + ": " + e.getMessage());
                return 0;
            }
        });
    }
    ...
    public @Nullable Long getPlayerIslandId(@NotNull Player player) {
        ...
        if (cachedId != null) {
            return Long.parseLong(cachedId.replace("island_", ""));
        }

        // Query island repository directly by owner UUID
        var island = oneblockIslandRepository.findByOwner(player.getUniqueId());

        // If not found in findAll cache, try by identifier lookup
        if (island == null) {
            island = oneblockIslandRepository.findByOwnerViaIdentifier(player.getUniqueId(), "island_");
        }

        if (island != null) {
            playerIslandCache.put(player.getUniqueId(), island.getIdentifier());
            return Long.parseLong(island.getIdentifier().replace("island_", ""));
        }
    ...
    public @Nullable Long getIslandIdAtLocation(@NotNull Location location) {
        ...
        // Find island by location - query database for islands near this location
    ...
    public boolean isOneBlockLocation(@NotNull Location location) {
        ...
        // Find the island and check if this is the oneblock location
        var island = oneblockIslandRepository.findByIdentifier("island_" + islandId);
        if (island == null) {
            return false;
        }
    ...
    private void loadNextIslandId() {
        // Get the highest island ID from database and increment
        var islands = oneblockIslandRepository.findAll();
        long maxId = 0;
        for (var island : islands) {
            try {
                long id = Long.parseLong(island.getIdentifier().replace("island_", ""));
                if (id > maxId) {
                    maxId = id;
                }
    ...
    private World getOrCreateOneblockWorld() {
        World world = Bukkit.getWorld(WORLD_NAME);
        if (world == null) {
            // Create world if it doesn't exist
            var creator = new org.bukkit.WorldCreator(WORLD_NAME);
    ...
    private Location calculateIslandLocation(World world, long islandId) {
        // Simple grid layout
        int gridSize = (int) Math.sqrt(islandId);
        int x = (int) (islandId % gridSize) * 1024;
        int z = (int) (islandId / gridSize) * 1024;
        return new Location(world, x, 100, z); // Y=100 for islands
    }

    private void generateInitialOneblock(World world, Location center) {
        // Create a simple platform with the oneblock in the center
        int platformSize = 5;
        ...
        // Place the oneblock
        world.getBlockAt(center).setType(Material.GRASS_BLOCK);

        // Add some basic items around
        world.getBlockAt(center.clone().add(1, 0, 0)).setType(Material.CHEST);
        world.getBlockAt(center.clone().add(-1, 0, 0)).setType(Material.CRAFTING_TABLE);
    }
    ...
    private void clearIslandArea(World world, OneblockIsland island) {
        ...
        // Clear blocks in the island area
    ...
    private void ensureSafeSpawn(Location location) {
        ...
        // Ensure there's a solid block below and air above
    ...
    public void showIslandInfo(@NotNull Player player) {
        ...
        player.sendMessage("§6========== Island Info ==========");
