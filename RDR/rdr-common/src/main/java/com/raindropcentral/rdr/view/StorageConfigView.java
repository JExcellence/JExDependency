 ...
 * @version 5.0.0
 */
public class StorageConfigView extends APaginatedView<StorageConfigView.ConfigEntry> {
    ...
    /**
     * Creates the storage config editor view.
     */
    public StorageConfigView() {
    ...
    public void onResume(
        ...
        if (result != null) {
            ...
            target.openForPlayer(
                ...
                Map.of("plugin", this.rdr.get(target))
            );
            return;
        }
    ...
    protected @NotNull CompletableFuture<List<ConfigEntry>> getAsyncPaginationSource(
        ...
        final YamlConfiguration configuration = YamlConfiguration.loadConfiguration(this.getConfigFile(this.rdr.get(context)));
        ...
        for (final Map.Entry<String, Object> valueEntry : configuration.getValues(true).entrySet()) {
            ...
            entries.add(new ConfigEntry(path, value, StorageConfigEditSupport.determineType(value)));
        }
    ...
    protected void onPaginatedRender(
        ...
        final int entryCount = this.countEntries(this.rdr.get(render));
    ...
    private void handleEntryEditClick(
        ...
        if (!this.rdr.get(clickContext).canChangeStorageSettings()) {
        ...
        clickContext.openForPlayer(
            StorageConfigValueAnvilView.class,
            Map.of(
                "plugin", this.rdr.get(clickContext),
                "configPath", entry.path(),
                "currentValue", StorageConfigEditSupport.formatValue(entry.value()),
    ...
    private boolean canAccessConfigEditor(
        ...
        return context.getPlayer().hasPermission(ADMIN_COMMAND_PERMISSION)
            && this.rdr.get(context).canChangeStorageSettings();
    }
    ...
    private @NotNull String previewValue(
        ...
        final String formatted = StorageConfigEditSupport.formatValue(value).replace('\n', ' ');
    ...
    protected record ConfigEntry(
        ...
        @NotNull StorageConfigEditSupport.SettingType settingType
    ) {
    }
