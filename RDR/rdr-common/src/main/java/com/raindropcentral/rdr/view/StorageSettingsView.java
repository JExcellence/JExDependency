...
public class StorageSettingsView extends BaseView {
    ...
    public void onFirstRender(
        ...
        final boolean canChangeSettings = owner && plugin.canChangeStorageSettings();
        render.layoutSlot('s')
        ...
        render.layoutSlot('t')
            .withItem(
                canChangeSettings
                    ? this.createTrustedItem(player, storage)
                    : owner
                        ? this.createEditionLockedActionItem(player)
                        : this.createLockedActionItem(player, "trusted")
            )
        ...
        render.layoutSlot('h')
            .withItem(
                canChangeSettings
                    ? this.createHotkeyItem(player, storage)
                    : owner
                        ? this.createEditionLockedActionItem(player)
                        : this.createLockedActionItem(player, "hotkey")
            )
            .onClick(clickContext -> {
                if (canChangeSettings) {
    ...
    private @NotNull ItemStack createEditionLockedActionItem(final @NotNull Player player) {
        return UnifiedBuilderFactory.item(Material.BARRIER)
            .setName(this.i18n("edition_locked.name", player).build().component())
            .setLore(this.i18n("edition_locked.lore", player).build().children())
            .addItemFlags(ItemFlag.HIDE_ATTRIBUTES)
            .build();
    }
