...
/**
 * Renders the storage admin currency-management view for RDR.
 *
...
public class AdminCurrencyView extends APaginatedView<AdminCurrencyView.DetectedCurrency> {
    ...
    private static final Logger LOGGER = Logger.getLogger(AdminCurrencyView.class.getName());
    ...
    protected @NotNull String getKey() {
        return "storage_admin_currency_ui";
    }

    /**
     * Returns the menu layout for this view.
     *
     * @return rendered layout with two paginated entry rows
     */
    @Override
    ...
    /**
     * Resolves paginated currency entries.
     *
     * @param context active menu context
     * @return async list of detected currencies
     */
    @Override
    ...
    /**
     * Renders a single currency entry.
     *
    ...
    /**
     * Renders static controls for the paginated currency view.
     *
    ...
    private @NotNull List<DetectedCurrency> detectCurrencies() {
        ...
        if (vaultEconomy != null) {
            final String vaultSingularName = this.normalizeDisplayValue(
                this.invokeStringMethod(vaultEconomy, "currencyNameSingular")
            );
            final String vaultPluralName = this.normalizeDisplayValue(
                this.invokeStringMethod(vaultEconomy, "currencyNamePlural")
            );
    ...
    private @Nullable DetectedCurrency extractJExCurrency(
        final @Nullable Object currencyEntity
    ) {
    ...
    private void handleCreateDefaultCurrencyClick(
    ...
    private boolean createDefaultCurrency(
        ...
        try {
            ...
            final CompletableFuture<Boolean> creationFuture = this.invokeCurrencyCreation(
                jexAdapter,
                currencyClass,
                currencyInstance,
                creator
            ...
            LOGGER.log(Level.WARNING, "Failed to create default RDR currency.", exception);
            return false;
        }
    }
    ...
    private @NotNull CompletableFuture<Boolean> invokeCurrencyCreation(
        ...
        try {
            ...
            final Method createCurrencyMethod = jexAdapter.getClass().getMethod("createCurrency", currencyClass);
    ...
    private @NotNull CompletableFuture<Boolean> asBooleanFuture(
        final @Nullable Object futureObject
    ) {
    ...
    private @Nullable Object resolveServiceProvider(
        final @NotNull String serviceClassName
    ) {
        try {
    ...
    private @NotNull String normalizeDisplayValue(
    ...
    private @NotNull ItemStack createSummaryItem(
        ...
        final boolean hasVault = this.resolveServiceProvider(VAULT_ECONOMY_CLASS) != null;
        final boolean hasJExEconomy = this.resolveServiceProvider(JEX_ADAPTER_CLASS) != null;

        return UnifiedBuilderFactory.item(Material.COMPASS)
            .setName(this.i18n("summary.name", player).build().component())
            .setLore(this.i18n("summary.lore", player)
                .withPlaceholders(Map.of(
                    "currency_count", detectedCurrencies.size(),
                    "vault_available", hasVault,
                    "jexeconomy_available", hasJExEconomy
                ))
                .build()
                .children())
            .addItemFlags(ItemFlag.HIDE_ATTRIBUTES)
