...
/**
 * Paginated selector for player-specific storage override editing.
 *
 ...
 * @version 5.0.0
 */
public class StorageAdminPlayerSelectView extends APaginatedView<StorageAdminPlayerSelectView.PlayerTargetEntry> {
     ...
     */
    public StorageAdminPlayerSelectView() {
    ...
    protected @NotNull String getKey() {
        return "storage_admin_player_select_ui";
    }
    ...
    protected @NotNull CompletableFuture<List<PlayerTargetEntry>> getAsyncPaginationSource(
        ...
        if (settingsService != null) {
            for (final Map.Entry<UUID, StorageAdminPlayerSettingsService.PlayerOverride> entry
                : settingsService.getPlayerOverrides().entrySet()) {
                final UUID playerId = entry.getKey();
                final StorageAdminPlayerSettingsService.PlayerOverride override = entry.getValue();
    ...
    protected void onPaginatedRender(
        ...
        final int configuredOverrides = plugin.getStorageAdminPlayerSettingsService() == null
            ? 0
            : plugin.getStorageAdminPlayerSettingsService().getPlayerOverrides().size();
    ...
    private @NotNull ItemStack createPlayerEntryItem(
        ...
        final int resolvedMaxStorages = onlinePlayer == null
            ? plugin.getMaximumStorages(entry.playerId(), config)
            : plugin.getMaximumStorages(onlinePlayer, config);
        final String resolvedMaxDisplay = this.formatMaxStorages(context.getPlayer(), resolvedMaxStorages);
        final String overrideMaxDisplay = override != null && override.maximumStorages() != null
            ? this.formatMaxStorages(context.getPlayer(), override.maximumStorages())
        ...
        return UnifiedBuilderFactory.item(entry.online() ? Material.PLAYER_HEAD : Material.SKELETON_SKULL)
            ...
            .setLore(this.i18n("entry.lore", context.getPlayer())
                .withPlaceholders(Map.of(
                    ...
                    "effective_max_storages", resolvedMaxDisplay,
    ...
    private @NotNull String resolveOnlineStatusPlaceholder(
        ...
        return this.i18n(key, viewer).build().getI18nVersionWrapper().asPlaceholder();
    }
    ...
    private @NotNull String formatMaxStorages(
        ...
        return this.i18n("entry.unlimited", player).build().getI18nVersionWrapper().asPlaceholder();
    }
