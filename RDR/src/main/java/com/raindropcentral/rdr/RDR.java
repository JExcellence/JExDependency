...
/**
 * Main plugin entry point for RDR.
 *
...
public class RDR extends JavaPlugin {
    ...
    /**
     * Allocates the shared platform and executor services before plugin enable.
     */
    @Override
    public void onLoad() {
        this.plugin = this;
    ...
    /**
     * Initializes persistence, commands, listeners, and storage views for the running plugin.
     */
    @Override
    ...
    public void onDisable() {
        this.getLogger().info("Disabling RDR: closing Hibernate");
    ...
    public @NotNull ConfigSection getDefaultConfig() {
        ...
        try {
            ...
            this.getLogger().warning("Failed to load RDR config from " + this.getDefaultConfigFile().getAbsolutePath() + ": " + e.getMessage());
    ...
    private void ensureDefaultConfigFile() {
        ...
        try {
            this.saveResource(CONFIG_FOLDER_PATH + "/" + CONFIG_FILE_NAME, false);
    ...
    /**
     * Returns the plugin instance exposed to helpers that expect a {@link JavaPlugin}.
     *
    ...
    private @Nullable Object resolveVaultEconomy() {
        ...
        try {
            ...
            final Object registration = this.getServer().getServicesManager().getRegistration(economyClass);
