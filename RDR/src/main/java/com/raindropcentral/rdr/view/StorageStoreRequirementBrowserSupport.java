/*
 * StorageStoreRequirementBrowserSupport.java
 *
 * @author ItsRainingHP
 * @version 5.0.0
 */
 ...
 */
final class StorageStoreRequirementBrowserSupport {
    ...
    private StorageStoreRequirementBrowserSupport() {
    }
    ...
    static boolean isRequirementMet(
        ...
        return StorageStorePricingSupport.isRequirementMet(plugin, player, requirement);
    }
    ...
    static int getProgressPercentage(
        ...
        return StorageStorePricingSupport.getProgressPercentage(plugin, player, requirement);
    }
    ...
    private static @NotNull List<String> buildDetailLines(
        final @NotNull RDR plugin,
        final @NotNull Player player,
        final @NotNull StorageStorePricingSupport.ResolvedStoreRequirement resolvedRequirement,
        final @NotNull AbstractRequirement requirement
    ) {
        final List<String> lines = new ArrayList<>();

        if (requirement instanceof CurrencyRequirement currencyRequirement) {
            final double bankedAmount = StorageStorePricingSupport.getStoredCurrencyProgress(
                plugin,
                player,
                resolvedRequirement
            );
            lines.add(
                "Balance: "
                    + StorageStorePricingSupport.formatCurrency(
                    plugin,
        ...
        if (requirement instanceof ItemRequirement itemRequirement) {
            ...
            for (int index = 0; index < Math.min(itemProgress.size(), MAX_ITEM_LINES); index++) {
                ...
                final int bankedAmount = StorageStorePricingSupport.getStoredItemAmount(
                    plugin,
                    player,
                    resolvedRequirement,
                    index
        ...
        if (requirement instanceof LocationRequirement locationRequirement) {
            ...
            if (locationRequirement.getRequiredCoordinates() != null) {
                ...
                lines.add(
                    "Distance: "
                        + StorageStorePricingSupport.formatAmount(locationRequirement.getCurrentDistance(player))
                        + " / "
                        + StorageStorePricingSupport.formatAmount(locationRequirement.getRequiredDistance())
                );
            }
        ...
        if (requirement instanceof PluginRequirement pluginRequirement) {
            ...
            for (final Map.Entry<String, Double> entry : pluginRequirement.getRequiredValues().entrySet()) {
                ...
                lines.add(
                    entry.getKey()
                        + ": "
                        + StorageStorePricingSupport.formatAmount(currentValue)
                        + " / "
                        + StorageStorePricingSupport.formatAmount(entry.getValue())
                );
                index++;
            }
            return lines;
        ...
        if (requirement instanceof TimedRequirement timedRequirement) {
            ...
            lines.add("Delegate: " + StorageStorePricingSupport.describeRequirement(plugin, timedRequirement.getDelegate()));
            return lines;
        }

        lines.add(StorageStorePricingSupport.describeRequirement(plugin, requirement));
        return lines;
    }
    ...
    private static void appendChildRequirementLines(
        ...
        for (int index = 0; index < Math.min(children.size(), MAX_CHILD_LINES); index++) {
            ...
            lines.add(
                (completed ? "[OK] " : "[X] ")
                    + StorageStorePricingSupport.describeRequirement(plugin, childRequirement)
            );
        }
