...
import com.raindropcentral.rdq.view.rank.ERankStatus;
import com.raindropcentral.rdq.view.rank.RankNode;
...
public final class RankPathOverview extends BaseView {
	...
	private final MutableState<Integer> panX          = this.mutableState(0);
	private final MutableState<Integer> panY          = this.mutableState(0);
	...
	private static final GridPosition               ROOT_ANCHOR  = new GridPosition(
		3,
		2
	);
	...
	protected @NotNull Map<String, Object> getTitlePlaceholders(final @NotNull OpenContext openContext) {
		...
		try {
			...
			if (previewMode) {
				rankTreeDisplayName = this.i18n(
					"preview_prefix",
					openContext.getPlayer()
				).build().append(rankTreeDisplayName.component());
			}
			return Map.of(
				"rank_tree_name",
	...
	public void onResume(
		...
		try {
			LOGGER.log(
				Level.FINE,
				"RankPathOverview resumed, refreshing cached data"
			);
	...
	private void initializeAndCacheData(
		...
		try {
			...
			if (! previewMode) {
				...
				final Map<String, ERankStatus> rankStatuses = this.calculateAllRankStatuses(
					rankHierarchy,
					ownedRanks,
					inProgressRanks,
					previewMode
				...
				LOGGER.log(
					Level.FINE,
					"Cached data: " + ownedRanks.size() + " owned ranks, " + inProgressRanks.size() + " pending ranks"
				);
			} else {
				final Map<String, ERankStatus> previewStatuses = new HashMap<>();
				for (final String rankId : rankHierarchy.keySet()) {
					final RRank rank = rankHierarchy.get(rankId).getRank();
					previewStatuses.put(
						rankId,
						rank.isInitialRank() ?
						ERankStatus.OWNED :
						ERankStatus.AVAILABLE
					);
				}
				this.nodeStatuses.set(
					previewStatuses,
	...
	private void addProgressionPathRanks(
		...
		final Map<String, RRank> rankMap = rankTree.getRanks().stream().collect(Collectors.toMap(
			RRank::getIdentifier,
			rank -> rank
		));
	...
	private @NotNull Map<String, ERankStatus> calculateAllRankStatuses(
	...
	private @NotNull TranslatedMessage extractRankTreeDisplayName(
		...
		return TranslationService.create(
			TranslationKey.of(rankTree.getDisplayNameKey()),
			player
		).build();
	}
	...
	private void handleNavigationClick(
		...
		final int currentX = this.panX.get(renderContext);
		final int currentY = this.panY.get(renderContext);
		this.panX.set(
			currentX + deltaX,
			renderContext
		);
	...
	private void renderDynamicRankGrid(
		...
		try {
			...
			LOGGER.log(
				Level.FINE,
				"Rendering dynamic grid with cached data: " + rankNodeHierarchy.size() + " nodes"
			);
	...
	private @NotNull ItemStack createDynamicSlotContent(
		...
		try {
			final int          offsetX          = this.panX.get(renderContext);
			final int          offsetY          = this.panY.get(renderContext);
	...
	private @Nullable String findRankIdAtWorldPosition(
	...
	private @Nullable ItemStack createConnectionLineItem(
	...
	private void positionSingleInitialRank(
		...
		worldPositions.put(
			initialRank.getRank().getIdentifier(),
			ROOT_ANCHOR
		);
		...
		LOGGER.log(
			Level.FINE,
			"Positioned single initial rank '" + initialRank.getRank().getIdentifier() + "' at center: " + ROOT_ANCHOR + " (slot " + slotNumber + ")"
		);
	...
	private void positionMultipleInitialRanks(
		...
		for (int i = 0; i < initialRanks.size(); i++) {
			...
			LOGGER.log(
				Level.FINE,
				"Positioned initial rank '" + initialRank.getRank().getIdentifier() + "' at: " + position + " (slot " + slotNumber + ")"
			);
			this.positionRankChildren(
				initialRank,
				position,
	...
	private void positionSingleChild(
		...
		if (! positionedRanks.contains(childNode.getRank().getIdentifier())) {
			final GridPosition childPosition = new GridPosition(
				parentPosition.x(),
				parentPosition.y() + GRID_STEP
			);
			worldPositions.put(
				childNode.getRank().getIdentifier(),
				childPosition
			);
			...
			LOGGER.log(
				Level.FINE,
				"Positioned single child '" + childNode.getRank().getIdentifier() + "' below parent at: " + childPosition + " (slot " + slotNumber + ")"
			);
			this.positionRankChildren(
				childNode,
				childPosition,
	...
	private void positionMultipleChildren(
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if (! positionedRanks.contains(child.getRank().getIdentifier())) {
				...
				LOGGER.log(
					Level.FINE,
					"Positioned child '" + child.getRank().getIdentifier() + "' at balanced position: " + childPosition + " (slot " + slotNumber + ")"
				);
				this.positionRankChildren(
					child,
					childPosition,
	...
	private @NotNull ItemStack createRankDisplayItem(
		...
		try {
			...
			final ERankStatus status = cachedStatuses.getOrDefault(
				rank.getIdentifier(),
				ERankStatus.LOCKED
			);
	...
	private @NotNull List<Component> buildRankDisplayLore(
		final @NotNull Player player,
		final @NotNull RRank rank,
		final @NotNull ERankStatus status,
		final boolean previewMode
	) {
	...
	private @NotNull ItemStack createConnectionLineDisplayItem(
		...
		try {
			final Map<String, ERankStatus> cachedStatuses = this.nodeStatuses.get(renderContext);
			final ERankStatus childStatus = cachedStatuses.getOrDefault(
				childNode.getRank().getIdentifier(),
				ERankStatus.LOCKED
			);
			...
			lore.add(this.i18n(
				...
				"rank_name",
				parentNode.getRank().getIdentifier()
			).build().component());
			lore.add(this.i18n(
				"connection.to",
				player
	...
	private void handleRankNodeClick(
		...
		final Map<String, ERankStatus> cachedStatuses     = this.nodeStatuses.get(renderContext);
		final ERankStatus rankStatus = cachedStatuses.getOrDefault(
			rankNode.getRank().getIdentifier(),
			ERankStatus.LOCKED
		);
		final ClickType clickType = clickContext.getClickOrigin().getClick();
		LOGGER.log(
			Level.FINE,
			"Handling rank click: " + rankNode.getRank().getIdentifier() + " - Preview: " + currentPreviewMode + " - Status: " + rankStatus + " - Click: " + clickType
		);
		...
		switch (rankStatus) {
			...
			case IN_PROGRESS -> this.handlePendingRankClick(
				clickContext,
				rankNode,
				clickType
			);
