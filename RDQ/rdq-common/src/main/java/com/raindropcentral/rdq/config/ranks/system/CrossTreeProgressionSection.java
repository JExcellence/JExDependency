...
public class CrossTreeProgressionSection extends AConfigSection {
	 ...
	 * If true, players can move from one rank tree to another.
	 */
	 ...
	 * If true, players can only switch to the same tier in another path.
	 */
	private Boolean requireSameRankTier;
	
	/**
	 * Whether downgrading to a lower rank is allowed when switching paths.
	 * If true, players can move to a lower rank in another path.
	 */
	 ...
	 *
	 * @return a list of rank tier integers; empty if not set
	 */
	public List<Integer> getSwitchRankTiers() {
		
		return
			this.switchRankTiers == null ?
			new ArrayList<>() :
			this.switchRankTiers;
	}
	
	/**
	 * Checks if switching is restricted to the same rank tier.
	 *
	 * @return true if switching requires the same rank tier, false otherwise
	 */
	public Boolean getRequireSameRankTier() {
		
		return
			this.requireSameRankTier == null ||
			this.requireSameRankTier;
	}
	
	/**
	 * Checks if downgrading to a lower rank is allowed when switching trees.
	 *
	 * @return true if downgrading is allowed, false otherwise
	 */
