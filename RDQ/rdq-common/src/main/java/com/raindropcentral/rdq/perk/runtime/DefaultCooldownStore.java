...
/**
 * Minimal default cooldown store used by legacy/event-triggered perks.
 * Thread-safe and simple: stores expiry timestamps per (player, perkId).
 */
public final class DefaultCooldownStore {

    private static final DefaultCooldownStore INSTANCE = new DefaultCooldownStore();

    // key: playerId -> (perkId -> expiryMillis)
    ...
    public void setCooldown(@NotNull Player player, @NotNull String perkId, long seconds) {
        ...
        final long expiry = System.currentTimeMillis() + (seconds * 1000L);
    ...
    public boolean isOnCooldown(@NotNull Player player, @NotNull String perkId) {
        final Map<String, Long> map = store.get(player.getUniqueId());
        if (map == null) return false;
        final Long expiry = map.get(perkId);
    ...
    public long getRemainingCooldown(@NotNull Player player, @NotNull String perkId) {
        ...
        final long remaining = (expiry - System.currentTimeMillis()) / 1000L;
        if (remaining <= 0) {
            map.remove(perkId);
            return 0L;
        }
    ...
    public void clearCooldown(@NotNull UUID playerId, @NotNull String perkId) {
        final Map<String, Long> map = store.get(playerId);
        if (map != null) {
            map.remove(perkId);
            if (map.isEmpty()) {
    ...
    public void clearExpired() {
        ...
        for (Map.Entry<UUID, Map<String, Long>> entry : store.entrySet()) {
            final Map<String, Long> inner = entry.getValue();
