...
/**
 * Detailed view for a single perk showing all information and action buttons.
 *
 * @author JExcellence
 * @version 2.0.0
 * @since 1.0.0
...
public class PerkDetailView extends BaseView {
    ...
    public void onFirstRender(@NotNull RenderContext render, @NotNull Player player) {
        try {
            ...
            final LoadedPerk perk = registry.get(id);
            if (perk == null) {
            ...
            // Calculate state
            final EPerkState state = calculateState(plugin, perk, player);
            final boolean isOwned = (state != LOCKED);
            final boolean isActive = (state == ACTIVE);
            ...
            if (!isOwned) {
                ...
                final int progressPercent = rPerk != null ? calculateOverallProgress(player, rPerk) : 0;
                final int[] requirementCounts = rPerk != null ? getRequirementCounts(player, rPerk) : new int[]{0, 0};
    ...
    private @NotNull ItemStack createPerkInfoItem(
        ...
        try {
            ...
            final Material material = Material.valueOf(perk.config().iconMaterial());
    ...
    private @NotNull EPerkState calculateState(
        ...
        try {
            ...
            /*if (perkManager.hasPlayerUnlockedPerk(player, perk.identifier())) {
                return AVAILABLE;
            }*/
            return LOCKED;
        } catch (final Exception e) {
    ...
    private int calculateOverallProgress(final @NotNull Player player, final @NotNull RPerk rPerk) {
        try {
            final List<RPerkUnlockRequirement> requirements = rPerk.getUnlockRequirementsOrdered();
            if (requirements.isEmpty()) {
                return 100;
            }

            final double totalProgress = requirements.stream()
    ...
    private int[] getRequirementCounts(final @NotNull Player player, final @NotNull RPerk rPerk) {
        try {
            final List<RPerkUnlockRequirement> requirements = rPerk.getUnlockRequirementsOrdered();
            final int total = requirements.size();
            final int completed = (int) requirements.stream()
