...
import com.raindropcentral.rdq2.database.entity.rank.*;
    ...
    CompletableFuture<Void> createRankTreesAsync(final @NotNull RankSystemState state,
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            for (Map.Entry<String, RankTreeSection> e : state.rankTreeSections().entrySet()) {
                ...
                tree.setFinalRankTree(cfg.getFinalRankTree());
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            for (Map.Entry<String, Map<String, RankSection>> entry : state.rankSections().entrySet()) {
                ...
                for (Map.Entry<String, RankSection> re : entry.getValue().entrySet()) {
                    ...
                    created.setInitialRank(cfg.getInitialRank());
                    created.setRankTree(tree);
                    rdq.getRankRepository().create(created);
...
/**
    ...
    private void createRankTrees(final @NotNull RankSystemState state) {
        for (final Map.Entry<String, RankTreeSection> entry : state.rankTreeSections().entrySet()) {
            try {
                final String treeId = entry.getKey();
    ...
    private void createRanks(final @NotNull RankSystemState state) {
        for (final Map.Entry<String, Map<String, RankSection>> treeEntry : state.rankSections().entrySet()) {
            ...
            for (final Map.Entry<String, RankSection> rankEntry : treeEntry.getValue().entrySet()) {
                try {
                    ...
                    state.ranks().computeIfAbsent(treeId, k -> new HashMap<>()).put(rankId, rank);
                } catch (Exception e) {
        ...
        for (final Map.Entry<String, Map<String, RankSection>> treeEntry : state.rankSections().entrySet()) {
            ...
            for (final Map.Entry<String, RankSection> rankEntry : treeEntry.getValue().entrySet()) {
                try {
                    ...
                    final List<String> prevRanks = cfg.getPreviousRanks().stream()
                            .map(prevId -> {
                                RRank prevRank = state.ranks().get(treeId).get(prevId);
                                return prevRank != null ? prevRank.getIdentifier() : null;
                            })
                            .filter(Objects::nonNull)
                            .collect(Collectors.toList());
                    final List<String> nextRanks = cfg.getNextRanks().stream()
                            .map(nextId -> {
                                RRank nextRank = state.ranks().get(treeId).get(nextId);
                                return nextRank != null ? nextRank.getIdentifier() : null;
                            })
                            .filter(Objects::nonNull)
                            .collect(Collectors.toList());
