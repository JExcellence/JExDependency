...
public final class RankSystemFactory {
    ...
    public void loadAndPersistRankSystem() {
        try {
            ...
            int treeCount = loadedState.rankTreeSections() != null ? loadedState.rankTreeSections().size() : 0;
            int rankCount = loadedState.rankSections() != null ? 
                loadedState.rankSections().values().stream()
                    .mapToInt(Map::size)
                    .sum() : 0;
