...
*/
/**
 * Service for triggering perks based on in-game events.
 *
 * Listens to Bukkit events and delegates perk triggering to the appropriate
 * perk runtimes based on event type and player eligibility.
 *
 * @author JExcellence
 * @version 1.0.4
 * @since 3.2.0
...
public class EventBasedPerkTrigger implements PerkTriggerService, Listener {

    private static final Logger LOGGER = CentralLogger.getLogger(EventBasedPerkTrigger.class.getName());
    ...
    public EventBasedPerkTrigger(@NotNull RDQ plugin, @NotNull DefaultPerkRegistry perkRegistry, @NotNull PerkAuditService auditService) {
        this.plugin = plugin;
    ...
    private void logThrottledFailure(String perkId, String fingerprint, String eventName, Throwable cause) {
        ...
        do {
            ...
            if (previous != 0L && (currentTimeMillis - previous) < EXCEPTION_THROTTLE_MILLIS) {
                return;
            }
        } while (!triggerFailureWindow.compareAndSet(previous, currentTimeMillis));
