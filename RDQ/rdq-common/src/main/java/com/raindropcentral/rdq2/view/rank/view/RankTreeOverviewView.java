...
public final class RankTreeOverviewView extends APaginatedView<RRankTree> {
	...
	private boolean hasPlayerSelectedRankTree(
		...
		return player.getPlayerRankPaths().stream()
			.anyMatch(rankPath -> Objects.equals(rankPath.getRankTree(), rankTree));
	}
	...
	private TreeDisplayState determineDisplayState(
		...
		return meetsPrerequisites ? TreeDisplayState.AVAILABLE : TreeDisplayState.LOCKED;
	}
	...
	private Material getMaterialForState(
		...
		if (state == TreeDisplayState.LOCKED) {
			return Material.BARRIER;
		}
		
		try {
			return Material.valueOf(rankTree.getIcon().getMaterial().toUpperCase());
	...
	private void handleRankTreeClick(
		...
		switch (state) {
			case AVAILABLE -> handleAvailableTreeClick(clickContext, rankTree, clickType, player);
			case LOCKED -> handleLockedTreeClick(clickContext, rankTree, clickType, player);
			case CURRENTLY_ACTIVE -> handleActiveTreeClick(clickContext, rankTree, clickType);
			case PREVIOUSLY_SELECTED -> handlePreviouslySelectedTreeClick(clickContext, rankTree, clickType);
		}
	}

	private void handleAvailableTreeClick(
		final @NotNull SlotClickContext clickContext,
		final @NotNull RRankTree rankTree,
		...
		if (currentActiveCount >= maximumRankPathsAllowed) {
	...
	private void handleLockedTreeClick(
		final @NotNull SlotClickContext clickContext,
		final @NotNull RRankTree rankTree,
		final @NotNull ClickType clickType,
		final @NotNull Player player
	...
	private void handleActiveTreeClick(
		final @NotNull SlotClickContext clickContext,
		final @NotNull RRankTree rankTree,
		final @NotNull ClickType clickType
	) {
	...
	private void handlePreviouslySelectedTreeClick(
		final @NotNull SlotClickContext clickContext,
		final @NotNull RRankTree rankTree,
		final @NotNull ClickType clickType
	) {
