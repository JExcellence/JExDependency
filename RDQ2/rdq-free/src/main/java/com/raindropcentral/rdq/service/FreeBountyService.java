...
public final class FreeBountyService implements BountyService {
    ...
    @Override
    public @NotNull CompletableFuture<List<RBounty>> getBountiesByCommissioner(@NotNull UUID commissionerUuid) {
        List<RBounty> result = bounties.values().stream()
                .filter(bounty -> bounty.getCommissionerUniqueId().equals(commissionerUuid))
                .toList();
    ...
    public @NotNull CompletableFuture<RBounty> createBounty(
        ...
        RBounty bounty = new RBounty(target.getUniqueId(), commissioner.getUniqueId(), rewardItems, rewardCurrencies);
    ...
    public boolean canCreateBounty(@NotNull Player player) {
        ...
        long count = bounties.values().stream()
                .filter(bounty -> bounty.getCommissionerUniqueId().equals(player.getUniqueId()))
                .count();
