...
if not "%~1"=="" (
    ...
    if not exist "%PLUGIN_DIR%" (
        ...
        echo      .\build-all.bat C:\Users\Privat\Desktop\Minecraft` Server\plugins
        echo      .\build-all.bat "C:\Users\Privat\Desktop\Minecraft Server\plugins"
        echo.
...
:build_modules
:: Step 1: RCommands - clean, publishToMavenLocal
echo.
call :print_step "1" "Building RCommands"
call :run_gradle_task "RCommands" "clean"
if !errorlevel! neq 0 exit /b 1

call :run_gradle_task "RCommands" "publishToMavenLocal"
if !errorlevel! neq 0 exit /b 1

:: Step 2: R18n - clean, publishToMavenLocal
echo.
call :print_step "2" "Building R18n"
call :run_gradle_task "R18n" "clean"
if !errorlevel! neq 0 exit /b 1

call :run_gradle_task "R18n" "publishToMavenLocal"
if !errorlevel! neq 0 exit /b 1
...
call :run_gradle_task "RCore" "build"
if !errorlevel! neq 0 exit /b 1
...
echo.
call :print_step "5" "Building RaindropQuests"
call :run_gradle_task "RaindropQuests" "clean"
if !errorlevel! neq 0 exit /b 1

call :run_gradle_task "RaindropQuests" "build"
if !errorlevel! neq 0 exit /b 1
...
echo   ^> RDQ Free: RaindropQuests\build\libs\RDQ-Free-6.0.0.jar
echo   ^> RDQ Premium: RaindropQuests\build\libs\RDQ-Premium-6.0.0.jar
...
if defined PLUGIN_DIR (
    ...
    if exist "JECurrency\build\libs\" (
        call :print_debug "JECurrency artifacts:"
        dir "JECurrency\build\libs\*.jar" /b 2>nul
    )
    if exist "RaindropQuests\build\libs\" (
        call :print_debug "RaindropQuests artifacts:"
        dir "RaindropQuests\build\libs\*.jar" /b 2>nul
    )
    
    call :deploy_jar "RCore\build\libs\RCore-Premium-2.0.0.jar" "RCore-Premium-2.0.0.jar"
    call :deploy_jar "JECurrency\build\libs\JECurrency-Premium-2.0.0.jar" "JECurrency-Premium-2.0.0.jar"
    call :deploy_jar "RaindropQuests\build\libs\RDQ-Premium-6.0.0.jar" "RDQ-Premium-6.0.0.jar"
    echo.
