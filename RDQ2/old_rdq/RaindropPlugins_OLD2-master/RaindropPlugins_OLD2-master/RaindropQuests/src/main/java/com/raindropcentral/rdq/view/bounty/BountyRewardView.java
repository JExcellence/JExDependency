...
public class BountyRewardView extends APaginatedView<RewardItem> {
	...
	protected CompletableFuture<List<RewardItem>> getAsyncPaginationSource(
		...
		if (
			...
			this.bounty.get(context).get().getRewardItems().isEmpty()
		) {
			final RewardItem pseudoItem = new RewardItem();
			pseudoItem.setContributorUniqueId(null);
			pseudoItem.setItem(
				this.buildPane(
	...
	private ItemStack buildPane(
		...
		return UnifiedBuilderFactory
			       ...
			       .setName(
				       I18n.create(
					       MessageKey.of(nameKey),
					       player
				       ).build().component()
			       )
			       .setLore(
				       I18n.create(
					       MessageKey.of(loreKey),
					       player
				       ).build().children()
			       )
