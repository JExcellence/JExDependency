...
import com.raindropcentral.rplatform.misc.heads.view.*;
...
public class RankPathOverview extends BaseView {
    ...
    private final State<RDQImpl> rdq = this.initialState("plugin");
    ...
    protected @NotNull Map<String, Object> getTitlePlaceholders(
        ...
        try {
            ...
            Message rankTreeDisplayName = this.extractRankTreeDisplayName(
                    rankTree,
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to generate title placeholders",
                    exception
            );
            return Map.of(
                    "rank_tree_name",
    ...
    public void onResume(
        ...
        try {
            LOGGER.log(
                    Level.FINE,
                    "RankPathOverview resumed, checking if data refresh is needed"
            );
            ...
            LOGGER.log(
                    Level.INFO,
                    "Refreshing cached data due to time elapsed: " + timeSinceRefresh + "ms"
            );
            final boolean previewMode = this.isPreviewMode.get(target);
            this.initializeAndCacheData(
                    target,
                    previewMode
            );

            this.renderDynamicRankGrid(
                    (RenderContext) target,
                    target.getPlayer(),
                    this.selectedRankTree.get(target),
                    previewMode
            );
        } catch (
                final Exception exception
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to refresh data on resume",
                    exception
            );
        }
    }
    ...
    public void onFirstRender(
        ...
        try {
            ...
            if (
                ...
                LOGGER.log(
                        Level.WARNING,
                        "RankTree is null, rendering error state"
                );
            ...
            this.renderStaticInterfaceComponents(
                    renderContext,
                    player,
                    rankTree,
                    previewMode
            ...
            LOGGER.log(
                    Level.SEVERE,
                    "Critical error during rank path overview render",
                    exception
            );
            this.renderCriticalErrorState(
                    renderContext,
                    player
            );
        }
    ...
    /**
     * Pre-loads and caches all necessary data to avoid database spam during navigation.
     */
    private void initializeAndCacheData(
            final @NotNull SlotClickContext renderContext,
        ...
        try {
            ...
            final RDQImpl plugin = this.rdq.get(renderContext);
            ...
            final Map<String, GridPosition> worldPositions = this.calculateRankWorldPositions(rankHierarchy);
            this.cachedWorldPositions.set(
                    worldPositions,
                    renderContext
            );

            if (!previewMode) {
                final Set<String> ownedRanks = this.loadOwnedRanks(
                        plugin,
                        rdqPlayer,
                        rankTree
                );
                final Set<String> inProgressRanks = this.loadInProgressRanks(
                        plugin,
                        rdqPlayer,
                        rankTree
                );

                this.cachedOwnedRanks.set(
                        ownedRanks,
                        renderContext
                );
                this.cachedInProgressRanks.set(
                        inProgressRanks,
                        renderContext
                );

                final Map<String, RankStatus> rankStatuses = this.calculateAllRankStatuses(
                        rankHierarchy,
                        ownedRanks,
                        inProgressRanks,
                        previewMode
            ...
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to initialize and cache data",
                    exception
            );
            this.cachedRankHierarchy.set(
                    new HashMap<>(),
    ...
    /**
     * Loads all ranks owned by the player in the given rank tree.
     */
    private @NotNull Set<String> loadOwnedRanks(
        ...
        try {
            ...
            final RPlayerRank playerRank = this.getPlayerRankForTree(
                    plugin,
                    rdqPlayer,
                    rankTree
            );
    ...
    /**
     * Recursively adds all ranks in the progression path leading to the target rank.
     */
    private void addProgressionPathRanks(
            final @NotNull RRank targetRank,
            final @NotNull RRankTree rankTree,
            final @NotNull Set<String> ownedRanks
    ) {

        final Map<String, RRank> rankMap = rankTree.getRanks().stream()
        ...
        this.addProgressionPathRanksRecursive(
                targetRank,
                rankMap,
                ownedRanks,
                visited
    ...
    private void addProgressionPathRanksRecursive(
            final @NotNull RRank currentRank,
            final @NotNull Map<String, RRank> rankMap,
            final @NotNull Set<String> ownedRanks,
            final @NotNull Set<String> visited
    ) {
    ...
    /**
     * Loads all ranks that are currently in progress for the player.
     */
    private @NotNull Set<String> loadInProgressRanks(
        ...
        try {
            ...
            for (final RRank rank : rankTree.getRanks()) {
                if (this.isRankInProgressDirect(
                        plugin,
                        rdqPlayer,
                        rank
                )) {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to load in-progress ranks",
                    exception
            );
            return new HashSet<>();
        }
    ...
    /**
     * Directly checks if a rank is in progress by querying upgrade progress.
     */
    private boolean isRankInProgressDirect(
            final @NotNull RDQImpl plugin,
            final @NotNull RDQPlayer rdqPlayer,
        ...
        try {
            ...
            for (final RRankUpgradeRequirement requirement : upgradeRequirements) {
                ...
                if (!progressList.isEmpty()) {
                    hasAnyProgress = true;
                    final RPlayerRankUpgradeProgress progress = progressList.get(0);
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to check if rank is in progress: " + rank.getIdentifier(),
                    exception
            );
            return false;
        }
    ...
    /**
     * Calculates status for all ranks based on cached data.
     */
    private @NotNull Map<String, RankStatus> calculateAllRankStatuses(
        ...
        for (final Map.Entry<String, RankNode> entry : rankHierarchy.entrySet()) {
            ...
            if (ownedRanks.contains(rankId)) {
                ...
                statuses.put(
                        rankId,
                        RankStatus.IN_PROGRESS
                );
            } else if (this.arePrerequisitesMetCached(
                    rankNode,
                    ownedRanks
            )) {
                statuses.put(
    ...
    /**
     * Checks prerequisites using cached owned ranks data.
     */
    private boolean arePrerequisitesMetCached(
    ...
    private @NotNull Message extractRankTreeDisplayName(
        ...
        return I18n.create(MessageKey.of(
    ...
    private void initializeViewOffsets(final @NotNull RenderContext renderContext) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.FINE,
                    "Initialized view offsets to zero"
            );
        } catch (final Exception exception) {
            LOGGER.log(
                    Level.WARNING,
                    "Failed to initialize view offsets",
                    exception
            );
            this.viewOffsetX.set(
                    0,
    ...
    private void renderStaticInterfaceComponents(
            final @NotNull RenderContext renderContext,
            final @NotNull Player player,
            final @NotNull RRankTree rankTree,
            final boolean previewMode
        ...
        try {
            ...
            if (previewMode) {
                this.renderPreviewModeIndicator(
                        renderContext,
                        player
                );
            }
            ...
            LOGGER.log(
                    Level.FINE,
                    "Static interface components rendered successfully"
            );
        } catch (final Exception exception) {
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to render static interface components",
                    exception
            );
        }
    }
    ...
    private void renderNavigationArrow(
        ...
        try {
            renderContext.slot(slotNumber)
                    .renderWith(() -> this.createNavigationArrowItem(
                            headProvider,
                            player,
                            direction
                    ))
                    .onClick(clickContext -> this.handleNavigationClick(
                            renderContext,
                            deltaX,
                            deltaY,
                            direction
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to render navigation arrow: " + direction,
                    exception
            );
        }
    }
    ...
    private @NotNull ItemStack createNavigationArrowItem(
        ...
        try {
            final ItemStack headItem = this.extractHeadFromProvider(
                    headProvider,
                    player
            );
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create navigation arrow item for: " + direction,
                    exception
            );
    ...
    private void renderBackButton(
        ...
        try {
            renderContext.slot(BACK_BUTTON_SLOT)
                    .renderWith(() -> this.createBackButtonItem(
                            player,
                            previewMode
                    ))
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to render back button",
                    exception
            );
        }
    }

    private @NotNull ItemStack createBackButtonItem(
            final @NotNull Player player,
            final boolean previewMode
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create back button item",
                    exception
            );
    ...
    private void renderCenterViewButton(
        ...
        try {
            renderContext.slot(CENTER_VIEW_SLOT)
                    .renderWith(() -> this.createCenterViewButtonItem(
                            player,
                            previewMode
                    ))
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to render center view button",
                    exception
            );
        }
    }

    private @NotNull ItemStack createCenterViewButtonItem(
            final @NotNull Player player,
            final boolean previewMode
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create center view button item",
                    exception
            );
            return UnifiedBuilderFactory.item(Material.COMPASS)
                    .setName(this.i18n(
                            "center_view.fallback",
                            player
                    ).build().component())
                    .addItemFlags(ItemFlag.HIDE_ATTRIBUTES)
                    .build();
    ...
    private void handleCenterViewClick(final @NotNull RenderContext renderContext) {
        ...
        LOGGER.log(
                Level.FINE,
                "Resetting view to center position"
        );
        this.viewOffsetX.set(
                0,
                renderContext
    ...
    private void renderPreviewModeIndicator(
            final @NotNull RenderContext renderContext,
            final @NotNull Player player
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to render preview mode indicator",
                    exception
            );
        }
    }
    ...
    private @NotNull ItemStack createPreviewModeIndicatorItem(final @NotNull Player player) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create preview mode indicator item",
                    exception
            );
            return UnifiedBuilderFactory.item(PREVIEW_MODE_MATERIAL)
                    .setName(this.i18n(
    ...
    private void renderDynamicRankGrid(
            final @NotNull RenderContext renderContext,
            final @NotNull Player player,
            final @NotNull RRankTree rankTree,
            final boolean previewMode
        ...
        try {
            final Map<String, RankNode> rankNodeHierarchy = this.cachedRankHierarchy.get(renderContext);
            final Map<String, GridPosition> worldPositionMapping = this.cachedWorldPositions.get(renderContext);

            LOGGER.log(
                    Level.FINE,
                    "Rendering dynamic grid with cached data: {} nodes",
                    rankNodeHierarchy.size()
            );

            for (final Integer slotNumber : ALL_RANK_SLOT_NUMBERS) {
                this.bindSlotToDynamicContent(
                        renderContext,
                        player,
                        rankTree,
                        slotNumber,
                        rankNodeHierarchy,
                        worldPositionMapping,
                        previewMode
                );
            }
            ...
            LOGGER.log(
                    Level.FINE,
                    "Dynamic rank grid rendered successfully"
            );
        } catch (final Exception exception) {
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to render dynamic rank grid",
                    exception
            );
            this.renderFallbackGrid(
                    renderContext,
                    player,
                    rankTree
            );
    ...
    private void bindSlotToDynamicContent(
            final @NotNull RenderContext renderContext,
            final @NotNull Player player,
            final @NotNull RRankTree rankTree,
            final int slotNumber,
            final @NotNull Map<String, RankNode> rankNodeHierarchy,
            final @NotNull Map<String, GridPosition> worldPositionMapping,
            final boolean previewMode
    ) {

        renderContext.slot(slotNumber)
                .renderWith(() -> this.createDynamicSlotContent(
                        slotNumber,
                        renderContext,
                        player,
                        rankTree,
                ...
                .updateOnStateChange(
                        this.viewOffsetX,
                        this.viewOffsetY,
                        this.dataRefreshTimestamp
                )
                .onClick(clickContext -> this.handleDynamicSlotClick(
                        clickContext,
                        slotNumber,
                        renderContext,
                        rankNodeHierarchy,
    ...
    private @NotNull ItemStack createDynamicSlotContent(
            final int slotNumber,
            final @NotNull RenderContext renderContext,
            final @NotNull Player player,
            final @NotNull RRankTree rankTree,
        ...
        try {
            final int offsetX = this.viewOffsetX.get(renderContext);
            final int offsetY = this.viewOffsetY.get(renderContext);
            ...
            final GridPosition worldPosition = new GridPosition(
                    slotGridPosition.x - offsetX,
                    slotGridPosition.y - offsetY
            );

            final String rankIdAtPosition = this.findRankIdAtWorldPosition(
                    worldPosition,
                    worldPositionMapping
            );
            if (rankIdAtPosition != null) {
                ...
                if (rankNode != null) {
                    return this.createRankDisplayItem(
                            player,
                            rankNode,
                            renderContext,
                            previewMode
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create dynamic slot content for slot " + slotNumber,
                    exception
            );
            return this.createBackgroundPane(player);
        }
    ...
    private GridPosition findGridPositionForSlot(final int slotNumber) {
    ...
    private ItemStack createConnectionLineItem(
        ...
        try {
            ...
            for (final Map.Entry<String, RankNode> entry : rankNodeHierarchy.entrySet()) {
                ...
                if (parentWorldPosition == null)
                    continue;

                for (final RankNode childNode : parentNode.children) {
                    final GridPosition childWorldPosition = worldPositionMapping.get(childNode.rank.getIdentifier());
                    if (childWorldPosition != null && this.isConnectionPosition(
                            worldPosition,
                            parentWorldPosition,
                            childWorldPosition,
                            parentNode,
                        ...
                        return this.createConnectionLineDisplayItem(
                                parentNode,
                                childNode,
                                player,
                                renderContext,
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create connection line item",
                    exception
            );
            return null;
        }
    ...
    private boolean isConnectionPosition(
        ...
        if (parentNode.children.size() == 1) {
            return this.isDirectConnectionPosition(
                    position,
                    parentPosition,
                    childPosition
            );
        }
        return this.isBalancedBranchingConnectionPosition(
                position,
                parentPosition,
                childPosition,
                parentNode,
    ...
    private boolean isBalancedBranchingConnectionPosition(
        ...
        final List<GridPosition> childPositions = this.extractChildPositions(
                parentNode,
                worldPositionMapping
        );
        ...
        final GridPosition branchingPoint = new GridPosition(
                parentPosition.x,
                parentPosition.y + branchingDistance
        );
    ...
    private @NotNull List<GridPosition> extractChildPositions(
        ...
        for (final RankNode childNode : parentNode.children) {
            final GridPosition childPosition = worldPositionMapping.get(childNode.rank.getIdentifier());
    ...
    private @NotNull ItemStack createConnectionLineDisplayItem(
        ...
        try {
            final Map<String, RankStatus> cachedStatuses = this.cachedRankStatuses.get(renderContext);
            final RankStatus childStatus = cachedStatuses.getOrDefault(
                    childNode.rank.getIdentifier(),
                    RankStatus.LOCKED
            );

            final Material connectionMaterial = this.getConnectionMaterialForStatus(childStatus);
            final Component connectionName = this.getConnectionNameForStatus(
                    childStatus,
                    player
            );
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create connection line display item",
                    exception
            );
    ...
    private @NotNull Material getConnectionMaterialForStatus(final @NotNull RankStatus status) {
    ...
    private @NotNull Component getConnectionNameForStatus(
            final @NotNull RankStatus status,
            final @NotNull Player player
    ) {

        return switch (status) {
            case OWNED -> this.i18n(
                    "connection.owned",
                    player
            ).build().component();
            case AVAILABLE -> this.i18n(
                    "connection.available",
                    player
            ).build().component();
            case IN_PROGRESS -> this.i18n(
    ...
    private void handleDynamicSlotClick(
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to handle dynamic slot click",
                    exception
            );
        }
    }

    private void handleRankNodeClick(
            final @NotNull SlotClickContext clickContext,
            final @NotNull RankNode rankNode,
            final @NotNull RenderContext renderContext
    ) {
        ...
        final Map<String, RankStatus> cachedStatuses = this.cachedRankStatuses.get(renderContext);
        final RankStatus rankStatus = cachedStatuses.getOrDefault(
        ...
        LOGGER.log(
                Level.FINE,
                "Handling rank click: " + rankNode.rank.getIdentifier() +
        ...
        if (currentPreviewMode) {
            this.handlePreviewModeRankClick(
                    player,
                    rankNode.rank
            );
            return;
        ...
        switch (rankStatus) {
            case OWNED -> this.handleOwnedRankClick(
                    player,
                    rankNode.rank
            );
            case AVAILABLE -> this.handleAvailableRankClick(
                    clickContext,
                    rankNode,
                    clickType
            );
            case IN_PROGRESS -> this.handleInProgressRankClick(
                    clickContext,
                    rankNode,
                    clickType
            );
    ...
    private void handlePreviewModeRankClick(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send preview mode rank click message",
                    exception
            );
            this.i18n(
    ...
    private void handleOwnedRankClick(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send owned rank click message",
                    exception
            );
            this.i18n(
    ...
    private void handleLockedRankClick(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send locked rank click message",
                    exception
            );
            this.i18n(
    ...
    private void createProgressTrackingEntries(
        ...
        for (final RRankUpgradeRequirement requirement : upgradeRequirements) {
            ...
            if (existingProgress.isEmpty()) {
                final RPlayerRankUpgradeProgress newProgress = new RPlayerRankUpgradeProgress(
                        rdqPlayer,
                        requirement
                );
                ...
                LOGGER.log(
                        Level.FINE,
                        "Created progress tracking for requirement " + requirement.getId()
                );
            }
        }
    }

    private void sendNoRequirementsMessage(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send no requirements message",
                    exception
            );
            this.i18n(
    ...
    private void sendProgressStartedMessage(
            final @NotNull Player player,
            final @NotNull RRank rank,
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send progress started message",
                    exception
            );
            this.i18n(
    ...
    private void sendProgressStartErrorMessage(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send progress start error message",
                    exception
            );
            this.i18n(
    ...
    private boolean checkAllRequirementsCompleted(
            final @NotNull RDQImpl plugin,
            final @NotNull Player player,
            final @NotNull Set<RRankUpgradeRequirement> upgradeRequirements
    ) {
        ...
        for (final RRankUpgradeRequirement requirement : upgradeRequirements) {
            ...
            if (progressList.isEmpty() || !progressList.get(0).isCompleted()) {
                return false;
            }
        }
        return true;
    ...
    private void redeemRank(
        ...
        try {
            final RDQImpl plugin = this.rdq.get(clickContext);
            ...
            final RRank newRank = rankNode.rank;

            LOGGER.log(
                    Level.INFO,
                    "Starting rank redemption for player " + player.getName() + " to rank " + newRank.getIdentifier()
            );

            this.handleRankRedemptionForTree(
                    plugin,
                    rdqPlayer,
                    rankTree,
                    newRank
            ...
            this.handleLuckPermsRankAssignment(
                    plugin,
                    player,
                    newRank
            );
            ...
            LOGGER.log(
                    Level.INFO,
                    "Successfully redeemed rank " + newRank.getIdentifier() + " for player " + player.getName()
            );
        } catch (final Exception exception) {
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to redeem rank " + rankNode.rank.getIdentifier() + " for player " + player.getName(),
                    exception
            );
            this.sendRankRedemptionErrorMessage(
                    player,
                    rankNode.rank
            );
        }
    ...
    private void sendRequirementsNotCompletedMessage(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send requirements not completed message",
                    exception
            );
            this.i18n(
    ...
    private void handleRankRedemptionForTree(
            final @NotNull RDQImpl plugin,
            final @NotNull RDQPlayer rdqPlayer,
            final @NotNull RRankTree rankTree,
        ...
        try {
            RPlayerRank existingRankInTree = this.getPlayerRankForTree(
                    plugin,
                    rdqPlayer,
                    rankTree
            );
            ...
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to handle rank redemption database operations",
                    exception
            );
            throw new RuntimeException(
                    "Database rank redemption failed",
                    exception
            );
        }
    }

    private void handleLuckPermsRankAssignment(
            final @NotNull RDQImpl plugin,
            final @NotNull Player player,
            final @NotNull RRank newRank
    ) {
        ...
        try {
            if (plugin.getLuckPermsService() != null) {
                ...
                if (luckPermsGroup != null && !luckPermsGroup.isEmpty()) {
                    this.removePlayerFromPreviousRankGroups(
                            plugin,
                            player,
                            newRank
                    );

                    //TODO plugin.getLuckPermsService().addPlayerToGroup(player.getUniqueId(), luckPermsGroup);
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to handle LuckPerms rank assignment",
                    exception
            );
        }
    }
    ...
    private void removePlayerFromPreviousRankGroups(
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to remove player from previous rank groups",
                    exception
            );
        }
    }
    ...
    private void markProgressEntriesCompleted(
        ...
        try {
            ...
            LOGGER.log(
                    Level.FINE,
                    "Marked progress entries as completed for rank " + rank.getIdentifier()
            );
        } catch (final Exception exception) {
            LOGGER.log(
                    Level.WARNING,
                    "Failed to mark progress entries as completed",
                    exception
            );
        }
    }
        ...
        try {
            ...
            return playerRanks.stream()
                    .filter(rank -> {
                        ...
                        return playerRankTree != null && Objects.equals(
                                playerRankTree,
                                rankTree
                        );
                    })
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to get player rank for tree",
                    exception
            );
            return null;
        }
    ...
    private void sendRankRedemptionErrorMessage(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send rank redemption error message",
                    exception
            );
            this.i18n(
    ...
    private void sendRankRedeemedMessage(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to send rank redeemed message",
                    exception
            );
            this.i18n(
    ...
    private void openRankRequirementsView(
            final @NotNull SlotClickContext clickContext,
            final @NotNull RankNode rankNode
    ) {
    ...
    private @NotNull Map<String, RankNode> buildRankNodeHierarchy(
            final @NotNull RRankTree rankTree
    ) {
        ...
        try {
            ...
            for (
                    final RRank rank : rankTree.getRanks()
            ) {
    ...
    private @NotNull Map<String, GridPosition> calculateRankWorldPositions(
            final @NotNull Map<String, RankNode> rankNodeHierarchy
    ) {
    ...
    private @NotNull List<RankNode> findInitialRanks(final @NotNull Map<String, RankNode> rankNodeHierarchy) {
        ...
        return rankNodeHierarchy.values().stream()
                .filter(node -> node.parents.isEmpty() || node.rank.isInitialRank())
    ...
    private void positionSingleInitialRank(
        ...
        final Integer slotNumber = RANK_SLOT_MAPPING.get(INITIAL_RANK_POSITION);
        LOGGER.log(
                Level.FINE,
                "Positioned single initial rank '" + initialRank.rank.getIdentifier() +
                        "' at center: " + INITIAL_RANK_POSITION + " (slot " + slotNumber + ")"
        );
    ...
    private void positionMultipleInitialRanks(
        ...
        for (
            ...
            final Integer slotNumber = RANK_SLOT_MAPPING.get(position);
            LOGGER.log(
                    Level.FINE,
                    "Positioned initial rank '" + initialRank.rank.getIdentifier() +
                            "' at: " + position + " (slot " + slotNumber + ")"
            );
    ...
    private void positionSingleChild(
        ...
        if (
            ...
            final GridPosition childPosition = new GridPosition(
                    parentPosition.x,
                    parentPosition.y + RANK_SPACING_DISTANCE
            );
            worldPositions.put(
                    childNode.rank.getIdentifier(),
                    childPosition
            ...
            final Integer slotNumber = RANK_SLOT_MAPPING.get(childPosition);
            LOGGER.log(
                    Level.FINE,
                    "Positioned single child '" + childNode.rank.getIdentifier() +
                            "' below parent at: " + childPosition + " (slot " + slotNumber + ")"
            );
    ...
    private void positionMultipleChildren(
        ...
        for (
            ...
            if (
                ...
                LOGGER.log(
                        Level.FINE,
                        "Positioned child '" + child.rank.getIdentifier() +
                                "' at balanced position: " + childPosition + " (slot " + slotNumber + ")"
                );
    ...
    private @NotNull ItemStack createRankDisplayItem(
        ...
        try {
            ...
            final Map<String, RankStatus> cachedStatuses = this.cachedRankStatuses.get(renderContext);
            final RankStatus status = cachedStatuses.getOrDefault(
                    rank.getIdentifier(),
                    RankStatus.LOCKED
            );

            final Material iconMaterial = this.extractRankIconMaterial(rank);
            final List<Component> lore = this.buildRankDisplayLore(
                    player,
                    rank,
                    status,
                    previewMode
            );
            final Component displayName = this.extractRankDisplayName(
                    player,
                    rank
            );
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to create rank display item",
                    exception
            );
    ...
    private @NotNull Material extractRankIconMaterial(
            final @NotNull RRank rank
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Invalid material for rank " + rank.getIdentifier() + ": " + rank.getIcon().getMaterial(),
                    exception
            );
    ...
    private @NotNull Component extractRankDisplayName(
            final @NotNull Player player,
            final @NotNull RRank rank
    ) {
        ...
        try {
            return I18n.create(
                    MessageKey.of(rank.getDisplayNameKey()),
                    player
            ).build().component();
        } catch (
                final Exception exception
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to extract localized rank display name",
                    exception
            );
            return this.i18n(
                            "rank.fallback_name",
    ...
    private boolean isRankInProgress(
            final @NotNull RDQPlayer rdqPlayer,
            final @NotNull RRank rank,
            final @NotNull Player player
    ) {
        ...
        try {
            if (
                    rank.getUpgradeRequirements().isEmpty()
            ) {
                return false;
            }

            final double overallProgress = progressManager.getRankOverallProgress(
                    player,
                    rdqPlayer,
                    rank
            );
            final boolean allCompleted = progressManager.areAllRequirementsCompleted(
                    player,
                    rdqPlayer,
                    rank
            );
    ...
    private void attemptRankRedemption(
            final @NotNull SlotClickContext clickContext,
            final @NotNull RankNode rankNode
    ) {
        ...
        try {
            ...
            final boolean allRequirementsCompleted = progressManager.areAllRequirementsCompleted(
                    player,
                    rdqPlayer,
                    rank
            );
            ...
            if (
                ...
                this.sendRequirementsNotCompletedMessage(
                        player,
                        rank
                );
            }
            ...
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to attempt rank redemption for " + rankNode.rank.getIdentifier(),
                    exception
            );
            this.i18n(
                    "error.general",
    ...
    private void startRankProgression(
            final @NotNull SlotClickContext clickContext,
            final @NotNull RankNode rankNode
    ) {
        ...
        try {
            ...
            if (
                    upgradeRequirements.isEmpty()
            ) {
                this.sendNoRequirementsMessage(
                        player,
                        rank
                );
                this.redeemRank(
            ...
            if (
                ...
                LOGGER.log(
                        Level.INFO,
                        "All requirements already completed for rank " + rank.getIdentifier() + ", directly redeeming for player " + player.getName()
                );
                this.redeemRank(
                        clickContext,
                        rankNode
            ...
            progressManager.initializeRankProgressTracking(
                    rdqPlayer,
                    rank
            );
            this.sendProgressStartedMessage(
                    player,
                    rank,
                    upgradeRequirements.size()
            );
            ...
            LOGGER.log(
                    Level.INFO,
                    "Started rank progression for player " + player.getName() + " on rank " + rank.getIdentifier()
            );
        } catch (final Exception exception) {
            LOGGER.log(
                    Level.SEVERE,
                    "Failed to start rank progression for " + rankNode.rank.getIdentifier(),
                    exception
            );
            this.sendProgressStartErrorMessage(
                    clickContext.getPlayer(),
                    rankNode.rank
            );
        }
    ...
    private void refreshCachedData(
        ...
        try {
            ...
            if (!previewMode) {
                LOGGER.log(
                        Level.FINE,
                        "Refreshing cached data after rank interaction"
                );
    ...
    private @NotNull List<Component> buildRankDisplayLore(
        ...
        try {
            ...
            LOGGER.log(
                    Level.WARNING,
                    "Failed to extract localized rank description",
                    exception
            );
            lore.add(this.i18n(
        ...
        lore.add(this.createStatusComponent(
                status,
                player
        ));
        lore.add(Component.empty());
    ...
    private void renderFallbackGrid(
        ...
        try {
            ...
            final List<Integer> centerSlots = List.of(
                    22,
                    21,
                    23,
                    13,
            ...
            for (
                    final Integer slot : ALL_RANK_SLOT_NUMBERS
            ) {
                if (!centerSlots.subList(
                        0,
                        Math.min(
                                ranks.size(),
                                centerSlots.size()
            ...
            LOGGER.log(
                    Level.INFO,
                    "Rendered fallback grid with " + Math.min(
                            ranks.size(),
                            centerSlots.size()
                    ) + " ranks"
            );
            ...
            LOGGER.log(
                    Level.SEVERE,
                    "Fallback grid rendering failed",
                    exception
            );
        }
    }
    ...
    private void renderCriticalErrorState(
            final @NotNull RenderContext renderContext,
            final @NotNull Player player
    ) {
        ...
        try {
            ...
            LOGGER.log(
                    Level.SEVERE,
                    "Critical error state rendering failed",
                    exception
            );
        }
    }
