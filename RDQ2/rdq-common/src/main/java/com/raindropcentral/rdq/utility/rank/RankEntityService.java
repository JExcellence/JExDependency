...
public final class RankEntityService {
    ...
    /**
     * Synchronously persists the entire rank system to the database.
     *
     * @param state the rank system state to persist
     */
    public void persistRankSystem(final @NotNull RankSystemState state) {
        try {
            LOGGER.info("Persisting rank system to database...");
            
            createDefaultRank(state);
            createRankTrees(state);
            createRanks(state);
            establishConnections(state);
            
            int treeCount = state.rankTrees() != null ? state.rankTrees().size() : 0;
            int rankCount = state.ranks() != null ? 
                state.ranks().values().stream()
                    .mapToInt(Map::size)
                    .sum() : 0;
            
            LOGGER.info(String.format("Persisted %d rank trees and %d ranks to database", treeCount, rankCount));
            
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Failed to persist rank system", e);
            throw new RuntimeException("Rank system persistence failed", e);
        }
    }

    private void createDefaultRank(final @NotNull RankSystemState state) {
        if (state.rankSystemSection() == null) return;
        final String id = state.rankSystemSection().getDefaultRank().getDefaultRankIdentifier();
        if (id == null || id.isBlank()) return;
        ...
        rdq.getRankRepository().save(created);
        state.setDefaultRank(created);
        LOGGER.log(Level.INFO, "Created default rank: {0}", id);
    }

    private void createRankTrees(final @NotNull RankSystemState state) {
        for (final Map.Entry<String, RankTreeSection> entry : state.getTreeSections().entrySet()) {
            try {
                final String treeId = entry.getKey();
                final RankTreeSection cfg = entry.getValue();

                RRankTree tree = rdq.getRankTreeRepository().findByAttributes(Map.of("identifier", treeId));
                if (tree == null) {
                    tree = new RRankTree(
                            treeId,
                            cfg.getDisplayNameKey(),
                    ...
                    rdq.getRankTreeRepository().save(tree);
                    LOGGER.log(Level.INFO, "Created rank tree: {0}", treeId);
                } else {
                    tree.setDisplayNameKey(cfg.getDisplayNameKey());
                    tree.setDescriptionKey(cfg.getDescriptionKey());
                    tree.setIcon(cfg.getIcon());
                    tree.setDisplayOrder(cfg.getDisplayOrder());
                    tree.setEnabled(cfg.getEnabled());
                    rdq.getRankTreeRepository().update(tree);
                    LOGGER.log(Level.INFO, "Updated rank tree: {0}", treeId);
                }
                state.addRankTree(treeId, tree);
            } catch (Exception e) {
                LOGGER.log(Level.SEVERE, "Failed to create/update rank tree: " + entry.getKey(), e);
            }
        }
    }

    private void createRanks(final @NotNull RankSystemState state) {
        for (final Map.Entry<String, Map<String, RankSection>> treeEntry : state.getRankSections().entrySet()) {
            final String treeId = treeEntry.getKey();
            final RRankTree tree = state.rankTrees().get(treeId);
            if (tree == null) {
                LOGGER.log(Level.WARNING, "Skipping ranks for missing tree: {0}", treeId);
                continue;
            }

            for (final Map.Entry<String, RankSection> rankEntry : treeEntry.getValue().entrySet()) {
                try {
                    final String rankId = rankEntry.getKey();
                    final RankSection cfg = rankEntry.getValue();
                    final String fullId = treeId + ":" + rankId;

                    RRank rank = rdq.getRankRepository().findByAttributes(Map.of("identifier", fullId));
                    if (rank == null) {
                        rank = new RRank(
                                fullId,
                                cfg.getDisplayNameKey(),
                        ...
                        rdq.getRankRepository().save(rank);
                        LOGGER.log(Level.INFO, "Created rank: {0}", fullId);
                    } else {
                        rank.setDisplayNameKey(cfg.getDisplayNameKey());
                        rank.setDescriptionKey(cfg.getDescriptionKey());
                        rank.setLuckPermsGroup(cfg.getLuckPermsGroup());
                        rank.setPrefixKey(cfg.getPrefixKey());
                        rank.setSuffixKey(cfg.getSuffixKey());
                        rank.setIcon(cfg.getIcon());
                        rank.setEnabled(cfg.getEnabled());
                        rank.setTier(cfg.getTier());
                        rank.setWeight(cfg.getWeight());
                        rank.setRankTree(tree);
                        rdq.getRankRepository().update(rank);
                        LOGGER.log(Level.INFO, "Updated rank: {0}", fullId);
                    }
                    state.addRank(treeId, rankId, rank);
                } catch (Exception e) {
                    LOGGER.log(Level.SEVERE, "Failed to create/update rank: " + treeId + ":" + rankEntry.getKey(), e);
                }
            }
        }
    }

    private void establishConnections(final @NotNull RankSystemState state) {
        for (final Map.Entry<String, Map<String, RankSection>> treeEntry : state.getRankSections().entrySet()) {
            final String treeId = treeEntry.getKey();
            for (final Map.Entry<String, RankSection> rankEntry : treeEntry.getValue().entrySet()) {
                try {
                    ...
                    final RankSection cfg = rankEntry.getValue();
                    final String fullId = treeId + ":" + rankId;
                    final RRank rank = state.ranks().get(treeId).get(rankId);
                    if (rank == null) continue;

                    final List<RRank> prevRanks = cfg.getPreviousRanks().stream()
                            .map(prevId -> state.ranks().get(treeId).get(prevId))
                            .filter(Objects::nonNull)
                            .collect(Collectors.toList());
                    final List<RRank> nextRanks = cfg.getNextRanks().stream()
                            .map(nextId -> state.ranks().get(treeId).get(nextId))
                            .filter(Objects::nonNull)
                            .collect(Collectors.toList());
                    ...
                    LOGGER.log(Level.SEVERE, "Failed to establish connections for rank: " + treeId + ":" + rankEntry.getKey(), e);
                }
            }
        }
    }
