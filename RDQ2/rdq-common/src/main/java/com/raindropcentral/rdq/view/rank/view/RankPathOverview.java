...
public final class RankPathOverview extends BaseView {
    ...
    public void onResume(@NotNull final Context origin, @NotNull final Context target) {
        try {
            LOGGER.log(Level.FINE, "RankPathOverview resumed, refreshing cached data");
            final boolean previewMode = this.isPreviewMode.get(target);
            this.initializeAndCacheData(target, previewMode);
            this.renderDynamicRankGrid((RenderContext) target, target.getPlayer(), this.selectedRankTree.get(target), previewMode);
        } catch (final Exception exception) {
    ...
    public void onFirstRender(final @NotNull RenderContext renderContext, final @NotNull Player player) {
        try {
            ...
            this.initializeAndCacheData(renderContext, previewMode);
    ...
    private void initializeAndCacheData(final @NotNull Context renderContext, final boolean previewMode) {
        try {
            ...
            this.nodeGraph.set(rankHierarchy, renderContext);
            final Map<String, GridPosition> worldPositions = this.calculateRankWorldPositions(rankHierarchy);
            this.nodeWorldPositions.set(worldPositions, renderContext);
            if (!previewMode) {
                final Set<String> ownedRanks = this.loadOwnedRanks(plugin, rdqPlayer, rankTree);
                final Set<String> inProgressRanks = this.loadInProgressRanks(plugin, rdqPlayer, rankTree, renderContext.getPlayer());
                this.ownedNodeIds.set(ownedRanks, renderContext);
                this.pendingNodeIds.set(inProgressRanks, renderContext);
                final Map<String, ERankStatus> rankStatuses = this.calculateAllRankStatuses(rankHierarchy, ownedRanks, inProgressRanks, previewMode);
                this.nodeStatuses.set(rankStatuses, renderContext);
                LOGGER.log(Level.FINE, "Cached data: " + ownedRanks.size() + " owned ranks, " + inProgressRanks.size() + " pending ranks");
            } else {
                ...
                for (final String rankId : rankHierarchy.keySet()) {
                    ...
                    previewStatuses.put(rankId, rank.isInitialRank() ? ERankStatus.OWNED : ERankStatus.AVAILABLE);
                }
                this.nodeStatuses.set(previewStatuses, renderContext);
            }
            this.lastRefreshAt.set(System.currentTimeMillis(), renderContext);
            ...
            this.nodeGraph.set(new HashMap<>(), renderContext);
            this.nodeWorldPositions.set(new HashMap<>(), renderContext);
            this.nodeStatuses.set(new HashMap<>(), renderContext);
            this.ownedNodeIds.set(new HashSet<>(), renderContext);
            this.pendingNodeIds.set(new HashSet<>(), renderContext);
        }
    }
    ...
    private void initializeViewOffsets(final @NotNull RenderContext renderContext) {
        try {
            this.panX.set(0, renderContext);
            this.panY.set(0, renderContext);
    ...
    private void handleNavigationClick(final @NotNull RenderContext renderContext, final int deltaX, final int deltaY, final @NotNull String direction) {
        ...
        this.panX.set(currentX + deltaX, renderContext);
        this.panY.set(currentY + deltaY, renderContext);
        LOGGER.log(Level.FINE, "Navigation " + direction + ": offset X=" + (currentX + deltaX) + ", Y=" + (currentY + deltaY));
    }
    ...
    private void bindSlotToDynamicContent(final @NotNull RenderContext renderContext, final @NotNull Player player, final @NotNull RRankTree rankTree, final int slotNumber, final @NotNull Map<String, RankNode> rankNodeHierarchy, final @NotNull Map<String, GridPosition> worldPositionMapping, final boolean previewMode) {
        renderContext.slot(slotNumber)
                ...
                .updateOnStateChange(this.panX, this.panY, this.lastRefreshAt)
