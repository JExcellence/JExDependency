...
public class PerkSectionConverter implements AttributeConverter<PerkSection, String> {
    ...
    public String convertToDatabaseColumn(final PerkSection perkSection) {
        try {
            if (perkSection == null) return null;

            var data = new PerkSectionData(
    ...
    @Override
    public PerkSection convertToEntityAttribute(final @Nullable String jsonString) {
        if (jsonString == null || jsonString.trim().isEmpty()) {
            return new PerkSection(new EvaluationEnvironmentBuilder());
        }

        try {
            var jsonNode = OBJECT_MAPPER.readTree(jsonString);
            var data = OBJECT_MAPPER.treeToValue(jsonNode, PerkSectionData.class);
            var perkSection = new PerkSection(new EvaluationEnvironmentBuilder());
