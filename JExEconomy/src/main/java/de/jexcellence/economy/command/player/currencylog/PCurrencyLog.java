...
public class PCurrencyLog extends PlayerCommand {
    ...
    /**
     * Reference to the main JExEconomyImpl plugin instance.
     */
     ...
     * @param jexEconomy the JExEconomyImpl plugin instance
     */
    public PCurrencyLog(
    ...
    private void handlePaginationRouting(final @NotNull Player player, final @NotNull String pageArg) {
        try {
            ...
            I18n.create(MessageKey.of("currency_log.invalid_page_format"), player).includePrefix().withPlaceholder("input", pageArg).sendMessage();
        }
    }
    ...
    private void handleViewAction(final @NotNull Player player, final @NotNull String[] args) {
        ...
        if (args.length > 1) {
            try {
                ...
                if (page < 1) {
                    I18n.create(MessageKey.of("currency_log.invalid_page_number"), player).includePrefix().withPlaceholder("page", args[1]).sendMessage();
                }
            } catch (NumberFormatException e) {
                I18n.create(MessageKey.of("currency_log.invalid_page_format"), player).includePrefix().withPlaceholder("input", args[1]).sendMessage();
            }
        }
    ...
    private CompletableFuture<Boolean> exportLogsToFile(Player player) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                File logsDir = new File(this.jexEconomyImpl.getImpl().getDataFolder(), "logs");
                if (!logsDir.exists()) {
                    logsDir.mkdirs();
                }
                ...
                Component successMessage = MINI_MESSAGE.deserialize(
                    ...
                    "<gray>Location: <white>plugins/JExEconomyImpl/logs/</white></gray>"
                );
                player.sendMessage(successMessage);
