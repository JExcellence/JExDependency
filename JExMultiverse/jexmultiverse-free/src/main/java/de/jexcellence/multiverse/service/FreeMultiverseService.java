...
/**
 * Free version of the Multiverse Service with limited functionality.
 * <p>
 * Limitations:
 * <ul>
 *   <li>Maximum 3 worlds</li>
 *   <li>Only DEFAULT and VOID world types available</li>
 *   <li>No PLOT world type</li>
 * </ul>
 * </p>
 *
 * @author JExcellence
 * @version 1.0.0
 */
public class FreeMultiverseService implements IMultiverseService {

    private static final Logger LOGGER = CentralLogger.getLoggerByName("FreeMultiverseService");

    private static final int MAX_WORLDS_FREE = 3;
    private static final List<MVWorldType> AVAILABLE_TYPES = Arrays.asList(MVWorldType.DEFAULT, MVWorldType.VOID);

    private static FreeMultiverseService instance;

    private final MVWorldRepository repository;
    private final WorldFactory worldFactory;

    private FreeMultiverseService(@NotNull MVWorldRepository repository, @NotNull WorldFactory worldFactory) {
        this.repository = repository;
        this.worldFactory = worldFactory;
    }

    /**
     * Initializes the Free Multiverse Service.
     *
     * @param repository   the world repository
     * @param worldFactory the world factory
     * @return the initialized service instance
     */
    public static FreeMultiverseService initialize(
            @NotNull MVWorldRepository repository,
            @NotNull WorldFactory worldFactory
    ) {
        if (instance == null) {
            instance = new FreeMultiverseService(repository, worldFactory);
            LOGGER.info("FreeMultiverseService initialized");
        }
        return instance;
    }
     ...
     */
    public static FreeMultiverseService getInstance() {
        if (instance == null) {
            throw new IllegalStateException("FreeMultiverseService not initialized. Call initialize() first.");
        }
        return instance;
    }
    ...
    public @NotNull CompletableFuture<MVWorld> createWorld(
        ...
        // Check world type availability
        if (!isWorldTypeAvailable(type)) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("Free version: World type " + type + " not available. Upgrade to Premium!")
            );
        }

        // Check world limit
        return isAtWorldLimit().thenCompose(atLimit -> {
            if (atLimit) {
                return CompletableFuture.failedFuture(
                        new IllegalStateException("Free version: Maximum " + MAX_WORLDS_FREE + " worlds. Upgrade to Premium!")
                );
            }
    ...
    public @NotNull CompletableFuture<Boolean> isAtWorldLimit() {
        return getWorldCount().thenApply(count -> count >= MAX_WORLDS_FREE);
    }
