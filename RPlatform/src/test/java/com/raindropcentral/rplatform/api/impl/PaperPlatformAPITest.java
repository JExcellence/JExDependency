...
class PaperPlatformAPITest {

    private ServerMock server;
    private JavaPlugin plugin;
    private PaperPlatformAPI api;
    private SchedulerStub scheduler;

    @BeforeEach
    void setUp() {
        server = MockBukkit.mock();
        plugin = MockBukkit.load(PaperTestPlugin.class);
        scheduler = new SchedulerStub();

        try (MockedStatic<ISchedulerAdapter> mocked = Mockito.mockStatic(ISchedulerAdapter.class)) {
            mocked.when(() -> ISchedulerAdapter.create(plugin, PlatformType.PAPER)).thenReturn(scheduler);
            api = new PaperPlatformAPI(plugin);
        }
    }
    ...
    @Test
    void metadataReportsPaperCapabilities() {
    ...
    void messagingDelegatesToPlayer() {
        ...
        Component message = Component.text("paper");
    ...
    void sendTitleBuildsAdventureTitle() {
        ...
        assertEquals(Duration.ofMillis(10 * 50L), built.times().fadeIn());
        assertEquals(Duration.ofMillis(40 * 50L), built.times().stay());
        assertEquals(Duration.ofMillis(5 * 50L), built.times().fadeOut());
    }
    ...
    void displayNameDelegatesToPlayer() {
        Player player = Mockito.mock(Player.class);
        Component displayName = Component.text("PaperUser");
    ...
    @Test
    void itemMetaNullBranchesRemainSafe() {
        ItemStack stack = new ItemStack(Material.AIR);
        assertNull(stack.getItemMeta());

        assertNull(api.getItemDisplayName(stack));
        Component displayName = Component.text("Name");
        assertSame(stack, api.setItemDisplayName(stack, displayName));
        assertSame(stack, api.setItemDisplayName(stack, null));

        List<Component> lore = api.getItemLore(stack);
        assertTrue(lore.isEmpty());
        assertThrows(UnsupportedOperationException.class, () -> lore.add(Component.text("fail")));

        List<Component> newLore = List.of(Component.text("Lore"));
        assertSame(stack, api.setItemLore(stack, newLore));
    }
    ...
    void itemDisplayNameAndLoreMutationsAreReflected() {
        ...
        Component displayName = Component.text("Sparkle");
        ...
        List<Component> readLore = api.getItemLore(stack);
        assertEquals(lore, readLore);
    ...
    void createPlayerHeadCopiesPlayerProfile() {
        Player player = Mockito.mock(Player.class);
        PlayerProfile profile = Bukkit.createProfile(UUID.randomUUID(), "PaperMock");
