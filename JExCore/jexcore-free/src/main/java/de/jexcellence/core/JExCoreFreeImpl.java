...
/**
 * Free edition delegate. Instantiates a {@link JExCore} orchestrator,
 * forwards lifecycle events, and installs {@code jexcore-stats} when the
 * statistics endpoint is configured.
 */
public final class JExCoreFreeImpl extends AbstractPluginDelegate<JExCoreFree> {

    private static final Logger LOGGER = Logger.getLogger(JExCoreFreeImpl.class.getName());

    private JExCore core;
    private StatisticsDelivery delivery;

    public JExCoreFreeImpl(@NotNull JExCoreFree plugin) {
        super(plugin);
    }
    ...
    public void onLoad() {
        try {
            this.core = new JExCore(getPlugin(), "Free") {
                @Override
                ...
                protected void onShutdown() {
                    if (JExCoreFreeImpl.this.delivery != null) {
                        JExCoreFreeImpl.this.delivery.shutdown(getPlugin());
                        JExCoreFreeImpl.this.delivery = null;
                    }
                }
            };
            this.core.onLoad();
        } catch (final Exception ex) {
            LOGGER.log(Level.SEVERE, "Failed to load JExCore Free", ex);
    ...
    public void onEnable() {
        if (this.core == null) {
            LOGGER.severe("Cannot enable - JExCore Free failed during onLoad.");
    ...
    public void onDisable() {
        try {
            ...
            LOGGER.log(Level.SEVERE, "Error during JExCore Free shutdown", ex);
        }
    }
